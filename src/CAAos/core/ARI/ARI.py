...
from CAAos.conf import ARsetup
...
class ARIcore():
    def __init__(self, samplingPeriod, unitP=None, unitV=None):

        # Panerai's ARI original parameters
        self.NsamplesBeforeImpuse = 9

        self.Ts = samplingPeriod  # sampling period
        ...
        # please define a function capable of creating these variables
        self.stepResponse = []
        self.timeVals = []
        self.responseLength = 0

    def calcStepResponse(self):
        # please redefine a function capable of creating these variables
        self.stepResponse = []
        self.timeVals = []
        self.responseLength = 0
    ...
    def calcARI(self):
        ...
        self.ARI_int = ARI_int
        self.ARI_frac = ARI_frac
        self.ARIbestFit = TiecksBestARI_int
...
class ARIanalysis(ARIcore):
    def __init__(self, transferFunction, samplingPeriod, unitP=None, unitV=None):
        ...
        # plot(self.timeVals[:self.nDuration], self.ARIbestFit, self.timeVals[:self.nDuration],  #      self.stepResponse[:self.nDuration], xlabel='time(s)', ylabel='step response', title='ARI best fit')
