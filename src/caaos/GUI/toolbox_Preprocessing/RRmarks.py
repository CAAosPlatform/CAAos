...
class signalRRmarksTab(signalTabWidget.signalTabWidget):
...
class heartRatePlotWidget(pg.GraphicsLayoutWidget):
    ...
    def setData(self, RRmarks, samplingRate_Hz):
        ...
        self.yData = np.reciprocal(np.diff(RRmarks).astype(float) / self.samplingRate_Hz) * 60.0
...
class signalPlotRRmarks(signalPlotWidget.signalPlot):
...
class plotArrayRRmarks(signalPlotWidget.plotArray):
