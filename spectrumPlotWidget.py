...
class spectrumPlot(signalPlot):
    ...
    def __init__(self,xData,xlabel='Frequency',xUnit='Hz',yData=[[np.array([1,2,3]),(255,0,0),'signal']],yUnit='adim.',title='Signal',logY=False,legend=False):
        channel=0
        super(spectrumPlot,self).__init__(channel,xData,xlabel,xUnit,yData,yUnit,title,legend)
        logX=False
        self.logY=logY
        self.setLogMode(logX,self.logY)
        
    def mysetTitle(self,yLabel):
        if yLabel is None:
            return
        try:
            self.title.setText(yLabel)
        except AttributeError:
            self.title = pg.TextItem(text=yLabel,anchor=(0,0), color=pyQtConf['textColor'], angle=0)
    ...
    def createFreqRegions(self,values=[[0.02,0.07],[0.07,0.20],[0.20,0.50]],colors=['#d5e5ff','#d7f4d7','#ffe6d5']):
        ...
        for i in range(len(values)):
            selection = pg.LinearRegionItem(values=values[i],orientation=pg.LinearRegionItem.Vertical,movable=False,brush=pg.mkBrush(color=colors[i]+alpha))
...
class plotArray(pg.GraphicsLayoutWidget):
    ...
    def markAvgRanges(self,plotNbr=0,avgValues=[1.0,1.0,1.0]):
        for range,avgVal in zip(['VLF', 'LF', 'HF'],avgValues):
            self.axes[plotNbr].hLine(avgVal,ARsetup.freqRangeDic[range])
