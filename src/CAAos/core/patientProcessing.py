...
from CAAos.core.TFA.TFA import transferFunctionAnalysis

__version__ = '0.2'


class patientProcessing():
    """
    ...
    def getVersion():
        ...
        **Example**
        ...
        >>> from patientProcessing import patientProcessing as pD
        >>> myCase=pD('data.EXP')
        '0.1'
        """
        return __version__
    ...
    def __init__(self):
        ...
        self.PPoperationsNode = None
        self.ARoperationsNode = None

    def setActiveModule(self, activeModule):

        # activeModule- Valid options: 'preprocessing', 'ARanalysis'
        if activeModule not in ['preprocessing', 'ARanalysis']:
            print('ERROR: setActiveModule unknown activeModule: %s ...' % activeModule)
            exit()
    ...
    def newJob(self, inputFile='./temp.exp'):
        """
        create a new job using the input file as data file. This function also loads data from the EXP and DAT  files.
        ...
        # add input file the tree
        dataNode = tools.ETaddElement(self.jobRootNode, tag='data', text=None, attribList=[['type', 'file']])
        tools.ETaddElement(dataNode, tag='inputFile', text=self.filePrefix + extension, attribList=[['type', self.DATAfileType]])
    ...
    def loadJob(self, inputFile_Job, activeModule):
        # activeModule- Valid options: 'preprocessing', 'ARanalysis'
        # parse JOB file into ETree


        [dir, _, extension] = tools.splitPath(inputFile_Job)

        if extension.lower() not in ['.job']:
        ...
        self.DATAfileName = dir + tools.getElemValueXpath(self.jobRootNode, xpath='data/inputFile', valType='str')
        ...
        for elem in self.jobRootNode.xpath('operationsFile'):
            ...
            # do not run now. it must run in order of appearance
            self.import_PPO_ARO_Operations(self.dirName + elem.text, pos, runOperations=False)
    ...
    def import_PPO_ARO_Operations(self, inputFile_PPO_ARO, elemPosition=None, runOperations=False):
        ...
        if (self.activeModule == 'preprocessing') and ext.lower() != '.ppo':
            print('ERROR: import_PPO_ARO_Operations in ->Preprocessing<- module requires .ppo file! exiting...')
            exit(-1)

        if (self.activeModule == 'ARanalysis') and ext.lower() != '.aro':
            print('ERROR: import_PPO_ARO_Operations in ->ARanalysis<- module requires .aro file! exiting...')
            exit(-1)
        ...
        **Example**
        ...
        if self.DATAfileType == 'PAR':
            ...
            for ch in range(self.nChannels):
                self.signals[ch].beat2beat(self.peakIdx, resampleRate_Hz=self.signals[ch].samplingRate_Hz,
                                           resampleMethod='linear')  # print(self.signals[ch].beat2beatData.max)
    ...
    def runPreprocessingOperations(self, operationsElem):
        ...
        **Note**
        ...
        for operation in operationsElem:
            if operation.tag not in ['resample', 'signalInfo', 'setLabel', 'setUnit', 'synchronize', 'findRRmarks', 'B2Bcalc', 'setType', 'calibrate',
                                     'LPfilter', 'interpolate', 'cropInterval', 'insertPeak', 'removePeak', 'SIGsave', 'B2Bsave', 'B2B_LPfilter']:
                print('Operation \'%s\' not recognized. Exiting...' % operation.tag)
                exit()

            if operation.tag == 'signalInfo':
            ...
            if operation.tag == 'setLabel':
                ...
                self.setSignalInfo(channel, label=label, register=False)

            if operation.tag == 'setUnit':
                unit = tools.getElemValueXpath(operation, xpath='unit', valType='str')
                channel = tools.getElemValueXpath(operation, xpath='channel', valType='int')
                print('Setting Unit channel=%d: %s' % (channel, unit))
                self.setSignalInfo(channel, unit=unit, register=False)
    ...
    def runARanalysisOperations(self, operationsElem):
        ...
        for operation in operationsElem:
            ...
            if operation.tag == 'MX':
                ...
                print('Computing Mx: useB2B=%s epochLength_s=%d blockLength_s=%d' % (str(useB2B), epochLength_s, blockLength_s))
    ...
    def setSignalInfo(self, channel, label=None, unit=None, sigType=None, register=True):
        self.signals[channel].setInfo(label=label, unit=unit, sigType=sigType, register=register)
    ...
    def saveTFAstatistics(self, filePath, plotFileFormat=None, coheTreshold=False, remNegPhase=False, register=True):
        ...
        if plotFileFormat is not None:
            if self.hasTFdata_L:
                self.TFA_L.savePlot(fileNamePrefix=os.path.splitext(filePath)[0] + '_Left', fileType=plotFileFormat.lower(),
                                    coheTreshold=coheTreshold, remNegPhase=remNegPhase, figDpi=250, fontSize=6)
            if self.hasTFdata_R:
                self.TFA_R.savePlot(fileNamePrefix=os.path.splitext(filePath)[0] + '_Right', fileType=plotFileFormat.lower(),
                                    coheTreshold=coheTreshold, remNegPhase=remNegPhase, figDpi=250, fontSize=6)
    ...
    def saveARIARMA(self, filePath, plotFileFormat=None, format='csv', register=True):
        ...
        if format.lower() == 'simple_text':
            ...
            if format.lower() == 'numpy':
                ...
                data = []
                if self.hasARIARMAdata_L:
                    data.append(('L', self.ARIARMA_L.ARI_int, self.ARIARMA_L.ARI_frac, self.ARIARMA_L.timeVals, self.ARIARMA_L.stepResponse,
                                 self.ARIARMA_L.ARIbestFit))
                if self.hasARIARMAdata_R:
                    data.append(('R', self.ARIARMA_R.ARI_int, self.ARIARMA_L.ARI_frac, self.ARIARMA_R.timeVals, self.ARIARMA_R.stepResponse,
                                 self.ARIARMA_R.ARIbestFit))
