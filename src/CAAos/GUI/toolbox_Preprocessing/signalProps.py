...
class signalPropsWidget(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        # plot area
        self.plotArea = signalPlotWidget.plotArray(self.data)
    ...
    def applyProps(self):
        for ch in self.signalTab.channels:
            channel = ch.channel
            self.data.signals[channel].setInfo(label=ch.label,unit=ch.unit,sigType=ch.sigType, register=True)
...
class signalPropsTab(signalTabWidget.signalTabWidget):
