...
from caaos.GUI.toolbox_ARanalysis.TFA import spectrumPlotWidget
...
class TFAWidget(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        # plot area
        self.plotAreaL = spectrumPlotWidget.plotArray(self.data, side='L', nCols=1)
        ...
        self.plotAreaR = spectrumPlotWidget.plotArray(self.data, side='R', nCols=1)
    ...
    def plotData(self, side='L'):
        ...
        if len(plotArea.axes) > 0:
            ...
            plotArea.addNewPlot(yData=[[TFAdata.getGain(freqRange='FULL'), pyQtConf['plotColors']['base'], 'Gain']],
                                yUnit='Gain\n[ ' + TFAdata.unitH + ' ]', title='', logY=False)
            plotArea.addNewPlot(yData=[[TFAdata.getPhase(freqRange='FULL') * 180 / np.pi, pyQtConf['plotColors']['base'], 'Phase']],
                                yUnit='Phase [ degree ]', title='', logY=False)
