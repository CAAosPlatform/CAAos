...
if darkTheme:
    pyQtConf =	{  # Dark theme
        ...
        "linearRegionBrush" : pg.mkBrush((200, 200, 200, 100)),
        "linearRegionPen"   : pg.mkPen(color=(255, 0, 0,255)),
        "plotLineWidth"     : 2.0,
        "peakMarkSymbol"    : 'o',
        "peakMarkSize"      : 5 ,
        "peakMarkColor"     : (255,0,0)
    }
else:
    pyQtConf =	{
        "backgroundColor"   : 'w',
        ...
        "linearRegionBrush" : pg.mkBrush((50, 50, 50, 50)),
        "linearRegionPen"   : pg.mkPen(color=(255, 0, 0,255)),
        "plotLineWidth"     : 2.0,
        "peakMarkSymbol"    : 'o',
        "peakMarkSize"      : 5 ,
...
class signalPlot(pg.PlotItem):
    ...
    def __init__(self,channel,xData=np.array([0,1,2]),xlabel='Time',xUnit='s',yData=[[np.array([1,2,3]),(255,0,0),'signal']],yUnit='V',title='signal',legend=False):
        ...
        self.signalCurve = [self.plot(x=self.xData,y=y,pen=pg.mkPen(color, width=pyQtConf['plotLineWidth'],name=label) ) for y,color,label in self.yData]
    ...
    def mysetTitle(self,title):
        if title is None:
            return
        try:
            self.title.setText('Ch %d) ' % self.channel + title)
        except AttributeError:
            self.title = pg.TextItem(text='Ch %d) ' % self.channel + title,anchor=(0,0), color=pyQtConf['textColor'], angle=0)
    ...
    def markPeaks(self,channel=0,peakIdx=None,valleyIdx=None):
        ...
        if peakIdx is not None:
            try:
                ...
                self.peakCurve = self.plot(self.xData[peakIdx],self.yData[channel][peakIdx],pen=None,
                                           symbol=pyQtConf['peakMarkSymbol'],symbolPen=None,symbolSize=pyQtConf['peakMarkSize'],
                                           symbolBrush=pyQtConf['peakMarkColor'])

        if valleyIdx is not None:
            try:
                self.valleyCurve.setData(self.xData[valleyIdx],self.yData[channel][valleyIdx])
            except AttributeError:
                self.valleyCurve = self.plot(self.xData[valleyIdx],self.yData[channel][valleyIdx],pen=None,
                                             symbol=pyQtConf['peakMarkSymbol'],symbolPen=None,symbolSize=pyQtConf['peakMarkSize'],
                                             symbolBrush=pyQtConf['peakMarkColor'])
    ...
    #replot data without creating a new curve. It changes xData and yData only. yData must be the same size of xData. If xData=None, the current xData is used
    # yData= list of numpy arrays
    ...
    def addSelection(self,minX,maxX,type='interval'):
        ...
        if type=='interval':
            ...
            self.selection = pg.LinearRegionItem(values=[minX,maxX],orientation=pg.LinearRegionItem.Vertical,bounds=[0,self.xData[-1]],pen=pyQtConf['linearRegionPen'],brush=pyQtConf['linearRegionBrush'])
...
class plotArray(pg.GraphicsLayoutWidget):
    ...
    def plotAllsignals(self,legend=False):
        ...
        for s in self.data.signals:
            self.addNewPlot(channel=s.channel,title=s.label,yData=[[s.data,pyQtConf['plotColors']['blue'],s.label]],yUnit=s.unit,legend=False)
    ...
    def addNewPlot(self,channel,title,yData,yUnit,legend=False):      
        ...
        newPlot = self.newPlotClass(channel,self.xData,'Time','s',yData,yUnit,title,legend=False)
