...
class signalPlot(pg.PlotItem):
    ...
    def markPeaks(self, channel=0, peakIdx=None, valleyIdx=None):
        ...
        if peakIdx is not None:
            try:
                #print(self.xData.shape, self.yData[channel].shape, peakIdx.shape)
                #print(peakIdx)
    ...
    def replot(self, xData, yData, yUnit):
        ...
        self.setLimits(xMin=0, xMax=self.xData[-1])
        self.update
...
class plotArray(pg.GraphicsLayoutWidget):
    ...
    def plotAllsignals(self, legend=False):
        ...
        for s in self.data.signals:
            self.addNewPlot(channel=s.channel, title=s.label,
                            yData=[[s.data[:self.Npoints], pyQtConf['plotColors']['blue'], 'Ch %d) ' % s.channel + s.label]], yUnit=s.unit,
                            legend=legend)
    ...
    def replotAllsignals(self):
        self.setXData(Npoints=self.data.signals[0].nPoints, samplingRate_Hz=self.data.signals[0].samplingRate_Hz, x0=0.0)
        for s in self.data.signals:
            self.axes[s.channel].replot(self.xData, [s.data[:self.Npoints]], s.unit)
