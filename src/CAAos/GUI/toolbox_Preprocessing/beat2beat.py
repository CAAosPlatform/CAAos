...
class signalBeat2beatWidget(QtWidgets.QWidget):
    ...
    def updateTab(self):
        ...
        if self.data.hasRRmarks:
            ...
            self.applyBeatButton.setText('Extract\nbeat-to-beat')
            self.applyBeatButton.setStyleSheet('color:rgb(0,0,0);background-color:rgb(192,255,208)')  # light green
        else:
            self.applyBeatButton.setEnabled(False)
            self.applyBeatButton.setText('Extract\nbeat-to-beat\n\nPlease find\n RR marks first')
            self.applyBeatButton.setStyleSheet('color:rgb(255,0,0)')
            self.plotArea.clearAll()
...
class plotArrayBeat2Beat(pg.GraphicsLayoutWidget):
    ...
    def plotAllsignals(self):
        ...
        for s in self.data.signals:
            ...
            self.addNewPlot(channel=s.channel, title='Ch %d) ' % s.channel + s.label, yData=yData, yUnit=s.unit)
    ...
    newPlotClass = signalPlotWidget.signalPlot
    ...
    def addNewPlot(self, channel, title, yData, yUnit):
        ...
        # add space to the left to fix alignment
        newPlot.getAxis("left").setWidth(pyQtConf['PlotLeftMargin'])
